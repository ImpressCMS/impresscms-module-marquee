...
require_once ICMS_ROOT_PATH.'/modules/marquee/admin/functions.php';
require_once ICMS_ROOT_PATH.'/modules/marquee/include/functions.php';
require_once ICMS_ROOT_PATH.'/modules/marquee/class/marquee_utils.php';
...
function AddEditMarqueeForm($marqueeid, $Action, $FormTitle, $contentvalue, $bgcolorvalue, $widthvalue, $heightvalue, $scrollamountvalue, $hspacevalue, $vspacevalue, $scrolldelayvalue, $directionvalue, $behaviourvalue, $alignvalue, $loopvalue, $stopvalue , $LabelSubmitButton, $sourcevalue = 'fixed')
{
	...
	$sform = new XoopsThemeForm($FormTitle, 'marqueeform', ICMS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/admin/index.php');
	 
	$source = new XoopsFormSelect(_AM_MARQUEE_SOURCE, 'source', $sourcevalue);
	$source->addOption('fixed', _AM_MARQUEE_SOURCE_FIXED);
	$fileslst = myglob(ICMS_ROOT_PATH.'/modules/marquee/plugins/', 'php');
...
switch ($op)
{
	...
	else
	{
		...
		{
			redirect_header('index.php', 2, _AM_MARQUEE_ERROR_ADD_MARQUEE.' MarqueeID');
			exit();
		}
	...
	if (isset($_POST['submit']) && $_POST['submit'] != '' )
	{
		...
		'',
			'marquee_align' => isset($_POST['align']) ? $_POST['align'] :
		0,
			'marquee_height' => $_POST['height'],
			'marquee_width' => $_POST['width'],
			'marquee_hspace' => isset($_POST['hspace']) ? $_POST['hspace'] :
		0,
			'marquee_scrolldelay' => $_POST['scrolldelay'],
			'marquee_stoponmouseover' => isset($_POST['stoponmouseover']) ? $_POST['stoponmouseover'] :
		0,
			'marquee_loop' => isset($_POST['loop']) ? $_POST['loop'] :
		0,
			'marquee_vspace' => isset($_POST['vspace']) ? $_POST['vspace'] :
		0,
			'marquee_content' => $_POST['content'],
			'marquee_source' => $_POST['source']));
		if (!$vres)
		{
			redirect_header('index.php', 1, _AM_MARQUEE_ERROR_WHILE_ADD_MARQUEE);
			exit();
		}
	...
	$class = 'even';
	$baseurl = ICMS_URL.'/modules/'.$xoopsModule->getVar('dirname').'/admin/index.php';
