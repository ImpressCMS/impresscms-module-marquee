...
function b_marquee_xfsection($limit, $dateformat, $itemssize)
{
	...
	{
		include_once ICMS_ROOT_PATH."/modules/xfsection/include/xfblock_groupaccess.php";
	}
	...
	while ($myrow = $xoopsDB->fetchArray($result) )
	{
		if (xfblock_checkAccess($myrow["groupid"]))
		{
			...
			$block[] = array('date' => formatTimestamp($myrow['published'], $dateformat),
				...
				'title' => $title,
				'link' => "<a href='".ICMS_URL.'/modules/xfsection/article.php?articleid='.$myrow['articleid']."'>".$title.'</a>');
		}
	}
	return $block;
}
