	...
	if ($newbb_version >= 2)
	{
		...
		foreach ($rows as $arr)
		{
			...
			$block[] = array('date' => formatTimestamp($arr['topic_time'], $dateformat),
				...
				'author' => $arr['uid'],
				'title' => $title,
				'link' => "<a href='" . ICMS_URL . "/modules/newbb/viewtopic.php?topic_id=" . $arr['topic_id'] . "&amp;post_id=" . $arr['post_id'] . "#forumpost" . $arr['post_id'] . "'>" . $title . "</a>");
		}
	}
	else
	{
		...
		while ($arr = $db->fetchArray($result))
		{
			...
			$block[] = array('date' => $time,
				...
				'author' => $tmpuser,
				'title' => $title,
				'link' => "<a href='".ICMS_URL."/modules/newbb/viewtopic.php?topic_id=".$arr['topic_id'] . "&amp;forum=". $arr['forum_id']."&amp;post_id=" . $arr['topic_last_post_id'] . "#forumpost" . $arr['topic_last_post_id'] . "'>" . $title . "</a>");
		}
