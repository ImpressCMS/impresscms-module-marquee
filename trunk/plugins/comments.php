	...
	if ($total > 0)
	{
		...
		foreach (array_keys($comments) as $i)
		{
			...
			$block[] = array('date' => formatTimestamp($comments[$i]->getVar('com_created'), $dateformat),
				...
				'title' => $title,
				'link' => "<a href='".ICMS_URL.'/modules/'.$module->getVar('dirname').'/'.$comment_config['pageName'].'?'.$comment_config['itemName'].'='.$comments[$i]->getVar('com_itemid').'&com_id='.$comments[$i]->getVar('com_id').'&com_rootid='.$comments[$i]->getVar('com_rootid').'&com_mode=thread&'.str_replace('&amp;', '&', $comments[$i]->getVar('com_exparams')).'#comment'.$comments[$i]->getVar('com_id')."'>".$title.'</a>');
		}
	}
	return $block;
}
