...
function b_marquee_wfsection($limit, $dateformat, $itemssize)
{
	...
	{
		// wfsection 1
		include_once ICMS_ROOT_PATH.'/modules/wfsection/include/groupaccess.php';
		global $xoopsDB;
		...
		while ($myrow = $xoopsDB->fetchArray($result) )
		{
			if (checkAccess($myrow["groupid"]))
			{
				...
				$block[] = array('date' => formatTimestamp($myrow['published'], $dateformat),
					...
					'title' => $title,
					'link' => "<a href='".ICMS_URL.'/modules/wfsection/article.php?articleid='.$myrow['articleid']."'>".$title.'</a>');
			}
		}
	}
	// wfsection 1 ou 2 ?
