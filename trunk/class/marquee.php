...
{
	include_once ICMS_ROOT_PATH.'/modules/marquee/class/PersistableObjectHandler.php';
}
...
class marquee extends marquee_Object {
	...
	function constructmarquee($uniqid = '')
	{
		...
		$scrolldelay = $this->getVar('marquee_scrolldelay') != 0 ? " scrolldelay=".$this->getVar('marquee_scrolldelay') :
		 '';
		$loop = $this->getVar('marquee_loop') != 0 ? " loop=".$this->getVar('marquee_loop') :
		 " loop='infinite'";
		...
		{
			include_once ICMS_ROOT_PATH.'/modules/marquee/plugins/'.$this->getVar('marquee_source').'.php';
...
class MarqueeMarqueeHandler extends marquee_XoopsPersistableObjectHandler {
	...
	function getHtmlMarqueesList($selectedmarquee = 0)
	{
		...
		foreach($tbl_marquee as $one_marquee)
		{
			...
			$content = xoops_trim(strip_tags($one_marquee->getVar('marquee_content'))) != '' ? xoops_substr(strip_tags($one_marquee->getVar('marquee_content')), 0, 50) :
			 $one_marquee->getVar('marquee_source');
